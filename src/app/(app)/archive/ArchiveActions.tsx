  ...
  return (
    <div className="card-soft p-3 sm:p-5 space-y-3">
      ...
      <div className="border-t pt-4">
        ...
        {error && <div role="alert" className="rounded-md bg-red-50 border border-red-200 text-red-700 text-sm p-3 mt-3 flex items-start gap-2"><AlertTriangle className="size-4 shrink-0 mt-0.5" /><span>{error}</span></div>}
        {msg && <div role="status" className="rounded-md bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm p-3 mt-3 flex items-start gap-2"><CheckCircle2 className="size-4 shrink-0 mt-0.5" /><span>{msg}</span></div>}
        <button onClick={uploadHistory} disabled={!file || !yearLabel || busy} className="btn-primary mt-3 w-full sm:w-auto">
