  ...
  return (
    <div className="space-y-4">
      ...
      {canEdit && (
        <div className="card-soft p-4">
          ...
          <div className="grid sm:grid-cols-4 gap-3">
            ...
            <input placeholder="Betrag (+/-)" className="input font-mono" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
          </div>
          <button onClick={add} disabled={busy} className="btn-primary mt-3">
      ...
      <div className="card-soft overflow-hidden">
        <div className="px-5 py-3 border-b font-semibold">Geplante Cashflows</div>
        <table className="data-table">
          <thead><tr><th>Datum</th><th>Beschreibung</th><th>Status</th><th className="text-right">Betrag</th>{canEdit && <th></th>}</tr></thead>
          <tbody>
            {entries.sort((a, b) => a.date.localeCompare(b.date)).map((e) => {
              ...
              return (
                <tr key={e.id} className={overdue ? "danger" : ""}>
                  ...
                  <td>{e.label}</td>
                  <td>{overdue ? <span className="chip chip-reminded">Überfällig</span> : <span className="chip chip-open">Geplant</span>}</td>
                  <td className={`text-right font-mono tabular ${e.amount >= 0 ? "amount-pos" : "amount-neg"}`}>{formatEUR(e.amount)}</td>
                  {canEdit && <td className="text-right"><button onClick={() => del(e.id)} className="btn-danger text-xs px-2 py-1"><Trash2 className="size-3.5" /></button></td>}
                </tr>
              );
            })}
            {entries.length === 0 && <tr><td colSpan={canEdit ? 5 : 4} className="text-center text-slate-500 py-8">Keine geplanten Cashflows.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
